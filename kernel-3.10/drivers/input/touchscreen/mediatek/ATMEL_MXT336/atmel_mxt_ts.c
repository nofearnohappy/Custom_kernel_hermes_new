...
#if defined(MXT_THR_ADJ)
#define MXT_T100_THR		30
				...
				u16 reg, u16 len, void *val, u8 *r_cache, u8 *r_cache_pa, u8 *w_cache, u8 *w_cache_pa, unsigned long flag)
{
	...
	//dev_err(&client->dev, "%s: i2c transfer(r)  (%x flag %x) reg %d len %d transferred %d\n",
		//__func__, client->addr, xfer[0].ext_flag, reg, len, transferred);
#endif
	...
	retry_read:
		...
		if (ret != num) {
			if (retry) {
				dev_err(&client->dev, "%s: i2c transfer(r) retry, reg %d\n", __func__, reg);
...
#if (DBG_LEVEL > 0)
		LCSH_DEBUG("[mxt] i2c transfer(r) reg %d len %d current %d transferred %d\n",
				...
				u16 reg, u16 len, const void *val, u8 *w_cache, u8 *w_cache_pa, unsigned long flag)
{
	...
	xfer.timing = 100;
	//dev_err(&client->dev, "%s: i2c transfer(w) (%x %x) reg %d len %d transferred %d\n",
		//__func__, client->addr , xfer.ext_flag, reg, len, transferred);
#endif
	...
	retry_write:
		...
		//print_hex_dump(KERN_DEBUG, "[mxt] w:", DUMP_PREFIX_NONE, 16, 1,
					//val+transferred, xfer.len, false);
#endif
	}
...
//zbl add
static int atmel_i2c_read(struct i2c_client *client, U16 addr, U16 len, U8 *rxbuf)
{
    u8 buffer[1]={0};
    u8 retry;
    u16 left = len;
    ...
    while ( left > 0 )
    {
        ...
        while ( i2c_transfer( client->adapter, &msg[0], 2 ) != 2 )
        {
            ...
            if ( retry == 5 )
            {
                printk("zbl I2C read 0x%X length=%d failed\n", addr + offset, len);
                printk("zbl I2C read 0x%X length=%d failed\n", addr + offset, len);
                return -1;
            }
        }
...
#if defined(CONFIG_ESD_CHECK_A_REGISTER)
static int esd_check_function(struct mxt_data *data)  //swf55
{
LCSH_DEBUG("swf55 esd_check_function \n ");
        int retval,i;
        char *read_data;
        size_t read_size;
	...
	u8 retval2;
        u8  T38DataPos = 2;
 	//t38_reg=waitful_data->T38_address;
       // struct i2c_client *client = waitful_data->client;
	...
	memset(read_data,1,read_size);
        ...
        LCSH_DEBUG("swf5555 read_data = 0x%2x\n",read_data[i]);
               ...
               // write_data_to_read(data);
                if(suspend_flag == 0)
                {
	             	retval2 = __mxt_read_reg(client, t38_reg+T38DataPos , read_size, read_data);
                          for(i=0;i<read_size;i++)
                              LCSH_DEBUG("swf5555cccc read_data = 0x%2x, retval2=%d\n",read_data[i],retval2);
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	int tool;
#if defined(MXT_THR_ADJ)
	static int old_reportnum = 0;
	static u8	adj_parameter[4] ={0 , 0 , 0 , 0};
	//	byte1: num_touch = 4;
	//  byte2: t100_thr_inc;
	//  byte3: t100_intthr_inc;
	//	byte4: t100_gain_inc;
#endif
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	u8 info[4];
	...
	if (id < 0) {
		dev_info(dev,"T100 [%d] SCRSTATUS : 0x%x\n",id,message[1]);
#if defined(MXT_THR_ADJ)
	if (id == -2) {

	   if( 0 == adj_parameter[0]){
		ret= __mxt_read_reg(data->client, data->T38_address + 60, sizeof(adj_parameter), adj_parameter);
		if(ret)
			return ret;
		}
		dev_info(dev,"[mxt] T38 adj parameter read out: num-0x%x, thr-0x%x, intthr-0x%x, gain-0x%x",
			adj_parameter[0],adj_parameter[1],adj_parameter[2],adj_parameter[3]);
		if(adj_parameter[0])
		{
			dev_info(dev,"[mxt] T38 adj parameter report num: old-%d, now-%d",old_reportnum , message[2]);
			
		  if(((message[2] > adj_parameter[0]) && (old_reportnum <= adj_parameter[0]))){
		  ret = __mxt_write_reg(data->client, data->T100_address+MXT_T100_THR, sizeof(adj_parameter[1]), &adj_parameter[1]);
		  ret = __mxt_write_reg(data->client, data->T100_address+MXT_T100_INTTHR, sizeof(adj_parameter[2]), &adj_parameter[2]);
		  ret = __mxt_write_reg(data->client, data->T100_address+MXT_T100_TCHHYST, sizeof(adj_parameter[3]), &adj_parameter[3]);
			msleep(20);
		        if(ret)
			  return ret;
			 dev_info(dev,"[mxt] T38 adj parameter change: thr-0x%x, intthr-0x%x, tchhyst-0x%x",
						adj_parameter[1] , adj_parameter[2] , adj_parameter[3]);
			}
		   else if ((message[2] <= adj_parameter[0]) && (old_reportnum > adj_parameter[0])){
			ret = __mxt_write_reg(data->client, data->T100_address+MXT_T100_THR, sizeof(data->t100_original_parameter[0]), &(data->t100_original_parameter[0]));
			ret = __mxt_write_reg(data->client, data->T100_address+MXT_T100_INTTHR, sizeof(data->t100_original_parameter[1]), &(data->t100_original_parameter[1]));
			ret = __mxt_write_reg(data->client, data->T100_address+MXT_T100_TCHHYST, sizeof(data->t100_original_parameter[2]), &(data->t100_original_parameter[2]));
			msleep(20);
			if(ret)
				return ret;			
			dev_info(dev,"[mxt] T38 adj parameter change: thr-0x%x, intthr-0x%x, tchhyst-0x%x",
				data->t100_original_parameter[0] , data->t100_original_parameter[1] , data->t100_original_parameter[2]);
				}      
			
			old_reportnum = message[2];
		...
		if (id == -2) {
			if (message[1] & 0x40)
				mxt_plugin_hook_t42(&data->plug,0x1);
			else
				mxt_plugin_hook_t42(&data->plug,0);
		}
#endif
		return;
	}
	...
	LCSH_DEBUG("[%u] status:%02X x:%u y:%u area:%02X amp:%02X vec:%02X\n",
		id,status,x, y,
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	info[MSG_T9_T100_STATUS] = status;
	info[MSG_T9_T100_AREA] = message[data->t100_aux_area] ;
	info[MSG_T9_T100_AMP] = message[data->t100_aux_ampl] ;
	info[MSG_T9_T100_VEC] = message[data->t100_aux_vect] ;
	ret = mxt_plugin_hook_t100(&data->plug, id, x, y, info);
	if (ret) {
		status = info[MSG_T9_T100_STATUS];
		message[data->t100_aux_ampl] = info[MSG_T9_T100_AMP];

		LCSH_DEBUG("[%u] status:%02X x:%u y:%u area:%02X amp:%02X vec:%02X (redefined)\n",
			id,status,x, y,
			(data->t100_aux_area) ? message[data->t100_aux_area] : 0,
			(data->t100_aux_ampl) ? message[data->t100_aux_ampl] : 0,
			(data->t100_aux_vect) ? message[data->t100_aux_vect] : 0);
	...
	if (status & MXT_T100_DETECT) {
		...
		input_report_abs(input_dev, ABS_MT_POSITION_X, x);
		input_report_abs(input_dev, ABS_MT_POSITION_Y, y);
//printk("swf999 X=%x,Y=%x,x=%x,y=%x\n",ABS_MT_POSITION_X,ABS_MT_POSITION_Y,x,y);/*swf add for menukey error*/
...
static void mxt_proc_t15_messages(struct mxt_data *data, u8 *msg)
{
	...
	for (key = 0; key < num_keys; key++) {
		...
		if (!curr_state && new_state) {
			printk("mxt swf9917 T15 key press: %u\n", key); /*swf add for menukey error*/
			__set_bit(key, &data->t15_keystatus);
			input_event(input_dev, EV_KEY,
					keymap[key], 1);
			sync = true;
		} else if (curr_state && !new_state) {
		//	printk( "mxt swf9918 T15 key release: %u\n", key);/*swf add for menukey error*/
			__clear_bit(key, &data->t15_keystatus);
			input_event(input_dev, EV_KEY,
					keymap[key], 0);
			sync = true;
...
static void mxt_proc_t61_messages(struct mxt_data *data, u8 *msg)
{
	...
	dev_info(dev, "T61 status 0x%x 0x%x\n",
			msg[0],
			msg[1]);
...
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
	...
	//	printk("swf99t100 pos, id=%u, min=%u,mxt=%u\n",report_id,data->T100_reportid_min,data->T100_reportid_max);/*swf add for menukey error*/
...
printk("mxt swf99t97  id=%u, min=%u,mxt=%u\n",report_id,data->T97_reportid_min,data->T97_reportid_max);
...
LCSH_DEBUG("ttttttt222 client:%p,data->T100_address:%x,t100_ori:%x",data->client,data->T100_address, (data->t100_original_parameter[1]));

	ret = __mxt_read_reg(data->client, data->T100_address + MXT_T100_THR, sizeof(data->t100_original_parameter[0]), &(data->t100_original_parameter[0]));
	if (error)
		return error;
	ret = __mxt_read_reg(data->client, data->T100_address + MXT_T100_INTTHR, sizeof(data->t100_original_parameter[1]), &(data->t100_original_parameter[1]));
	if (error)
		return error;
	ret = __mxt_read_reg(data->client, data->T100_address + MXT_T100_TCHHYST, sizeof(data->t100_original_parameter[2]), &(data->t100_original_parameter[2]));
	if (error)
		return error;

        dev_info(&client->dev,"[mxt] T38 read parameter change: thr-0x%x, intthr-0x%x, tchhyst-0x%x",
		data->t100_original_parameter[0] , data->t100_original_parameter[1] , data->t100_original_parameter[2]);
...
static int mxt_initialize_t100_input_device(struct mxt_data *data)
{
	...
	/* For multi touch */
	error = input_mt_init_slots(input_dev, data->num_touchids,0/*, INPUT_MT_DIRECT*/);  /*******sunwf modify*******/
	if (error) {
...
//susan
static ssize_t mxt_devcfg_crc_show(struct device *dev,struct device_attribute *attr, char *buf)
{
//	int ret;
	struct mxt_data *data = dev_get_drvdata(dev);
	mxt_update_crc(data, MXT_COMMAND_REPORTALL, 1);
	if (!data->info)
		return 0;
		//ret = mxt_update_file_name(dev, &data->cfg_name, buf, sizeof(data->cfg_name)/*count*/, data->alt_chip);
//	if (ret)
//		return ret;
...
#if defined(CONFIG_ESD_CHECK_A_REGISTER)
LCSH_DEBUG("swf55 mxt_update_fw_store start \n");
...
static DEVICE_ATTR(devcfg_crc, S_IRUGO, mxt_devcfg_crc_show, NULL);//susan
...
static struct attribute *mxt_attrs[] = {
	&dev_attr_devcfg_crc.attr, //susan
...
static struct mxt_platform_data *mxt_parse_dt(struct i2c_client *client)
{
	...
	*/
	...
	if (prop) {
		...
		keymap[T93_KEY][0] = KEY_POWER; //swf 66
#endif
...
static int mxt_initialize_t9_input_device(struct mxt_data *data)
{
	...
	/* For multi touch */
	num_mt_slots = data->num_touchids + data->num_stylusids;
	error = input_mt_init_slots(input_dev, num_mt_slots,0/*, INPUT_MT_DIRECT*/); /********* sunwf modify *********/
	if (error) {
		dev_err(dev, "Error %d initialising slots\n", error);
		goto err_free_mem;
	}
...
static ctp_vender_open (struct inode* inode, struct file* file) 
{
    return single_open(file, ctp_vender_open_show, inode->i_private);
}
...
static const struct file_operations ctp_vender_fops = {
    .open = ctp_vender_open,
    .read = seq_read,
};
#endif
...
static void tp_devinfo_init(struct mxt_data *data)
{
	...
	if(NULL != devinfo_tp)
	{
		...
		devinfo_check_add_device(devinfo_tp);
 		...
 		LCSH_DEBUG("swf77 failed to create tp deviinfo **\n");
	}
}
#endif
...
static int mxt_probe(struct i2c_client *client,	const struct i2c_device_id *id)	//__devinit mxt_probe //swfprobe
{
	struct mxt_data *data;
	int error;
        int retval;  //swf55
LCSH_DEBUG("*****sunwf111111 func = %s line = %d ******\n",__func__,__LINE__);
	dev_info(&client->dev, "%s: driver version 0x%x\n", 
			__func__,DRIVER_VERSION);

	data = kzalloc(sizeof(struct mxt_data), GFP_KERNEL);
#if defined(CONFIG_ESD_CHECK_A_REGISTER)
       // waitful_data = kzalloc(sizeof(struct mxt_data), GFP_KERNEL);  //swfesd
#endif
	if (!data) {
		dev_err(&client->dev, "Failed to allocate memory\n");
		return -ENOMEM;
...
#if defined(CONFIG_MXT_I2C_DMA)
	client->addr |= I2C_RS_FLAG | I2C_ENEXT_FLAG | I2C_DMA_FLAG;
	dma_set_coherent_mask(&client->dev, DMA_BIT_MASK(32));	//lc--litao--add-20150523-for 32bit DMA
...
LCSH_DEBUG("*****sunwf111111 func = %s line = %d ******\n",__func__,__LINE__);
	...
	if ( IS_ERR(esd_thread) ) {
		...
		LCSH_DEBUG(" %s: failed to create kernel thread: %d\n",__func__, retval);
		return retval;
	}
...
//LC--zbl--add--20150520 for kernel3.1 proc node start
	...
	if(proc_create("ctp_version", 0444, NULL, &ctp_vender_fops) == NULL)
	{
                printk("create_proc_entry  failed\n");
		return -1;
        }	
#if 0
    g_ctp_proc = create_proc_entry("tp_info", 0444, NULL);
    if (g_ctp_proc == NULL)
    {
 	printk("create_proc_entry failed\n"); 
    }
...
static int mxt_remove(struct i2c_client *client)//__devexit mxt_remove /**************sunwf modify**************/
{
	struct mxt_data *data = i2c_get_clientdata(client);
...
#if defined(CONFIG_ESD_CHECK_A_REGISTER)
	suspend_flag = 1;//swf55 stop esd check
#endif
	if (mxt_suspend(&mxt->client->dev) != 0)
		dev_err(&mxt->client->dev, "%s: failed\n", __func__);
}
...
#if 1//defined(CONFIG_MXT_EXTERNAL_MODULE)
#include "atmel_mxt_ts_mtk_interface.c"
#else
static struct i2c_driver mxt_driver = {
	...
	.probe		= mxt_probe,
	.remove		= mxt_remove,//__devexit_p(mxt_remove),      /**************sunwf modify**************/
