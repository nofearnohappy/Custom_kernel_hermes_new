...
#define PI_FLAG_BINDING					(1<<12)
#define PI_FLAG_OPTION					(1<<13)
#define PI_FLAG_GLOVE					(1<<14)
#define PI_FLAG_STYLUS					(1<<15)
#define PI_FLAG_WAKEUP					(1<<16)
#define PI_FLAG_GESTURE					(1<<17)

//more than 16 is high mask

#define PI_FLAG_WORKAROUND_HALT			(1<<31)

#define PI_FLAG_MASK_LOW			(0xf)
...
struct pi_observer{
	...
	struct reg_config com;  //set reg instruction
...
static int get_gesture_trace_data(struct plugin_proci *p, u8 symbol, u8 * buf)
{
	...
	if (p->get_diagnostic_data) {
		...
		buf[1] = 0;
		do {
			//get stroke count,save to offset 3
			ret = p->get_diagnostic_data(p->dev, MXT_T6_DEBUG_GESTRUE, 0, 0, 1, buf + GESTURE_TRACE_STROKE_CNT_INDEX, 10, 5);
			if (ret == 0) {
				buf[GESTURE_TRACE_STROKE_CNT_INDEX] &= 0x1F;
				if(strcnt && strcnt == buf[GESTURE_TRACE_STROKE_CNT_INDEX])
					break;
				strcnt = buf[GESTURE_TRACE_STROKE_CNT_INDEX];
		...
		if (ret == 0) {
			if (strcnt && strcnt < MAX_GESTURE_TRACE_STROKE) {
				...
				ret = p->get_diagnostic_data(p->dev, MXT_T6_DEBUG_GESTRUE, 0, 1, (strcnt << 2), buf + GESTURE_TRACE_STROKE_CNT_INDEX + 1, 10, 5);
			}else {
...
static int binding_list_config_set(struct plugin_proci *p, const struct reg_config *rset)
{
	struct pi_config *cfg = p->cfg;
	struct reg_config *rcfg;

	int i,num,ret = -EINVAL;

	rcfg = cfg->reg_cfg[PI_BINDING];
	num = cfg->num_reg_cfg[PI_BINDING];
	for (i = 0; i <= num; i++) {
		if (rcfg[i].reg ==  rset->reg) {
			if (rset->len) {
				memcpy(&rcfg[i], rset, sizeof(struct reg_config));
				rcfg[i].flag = BIT_MASK(P_COMMON);
			}else {
				rcfg[i].flag = 0;
			}
			ret = 0;
...
static int option_list_config_set(struct plugin_proci *p, int select_id)
{
	struct pi_config *cfg = p->cfg;
	struct reg_config *rcfg;
	...
	rcfg[select_id].flag = BIT_MASK(P_COMMON);
...
static long plugin_proci_pi_post_process_messages(struct plugin_proci *p, unsigned long pl_flag)
{
	...
	struct pi_descriptor dpr[] = {
		...
		/*{PL_FUNCTION_FLAG_BINDING,PI_FLAG_BINDING,PI_BINDING},
		{PL_FUNCTION_FLAG_OPTION,PI_FLAG_OPTION,PI_OPTION}*/};
	...
	if (test_flag(PI_FLAG_RESET,&obs->flag)) {
		if (test_and_clear_flag(PI_FLAG_MASK_FUNC,&obs->flag))	 {  //double reset
			dev_info2(dev, "mxt pi double reset pl_flag=0x%lx flag=0x%lx\n",	 pl_flag, obs->flag);
			if (p->reset(p->dev) != 0)
				p->set_and_clr_flag(p->dev, PL_STATUS_FLAG_NEED_RESET, 0);
			return 0;
		}
...
static struct reg_config mxt_binding_list[] = {

	{.reg = MXT_PROCI_GRIPSUPPRESSION_T40,0,
		.offset = 0,.buf = {0x1}, .len = 1, .mask = 0x1, .flag = /*BIT_MASK(P_COMMON)*/0, .tag = 0, .sleep = 0},
};

static struct reg_config mxt_option_cfg[] = {

	{.reg = MXT_PROCI_TOUCHSUPPRESSION_T42,0,
		.offset = 0,.buf = {0x1}, .len = 1, .mask = 0x1, .flag = 0, .tag = 0, .sleep = 0},
	{.reg = MXT_PROCI_TOUCHSUPPRESSION_T42,0,
		.offset = 0,.buf = {0x1}, .len = 1, .mask = 0x1, .flag = 0, .tag = 0, .sleep = 0},
	{.reg = MXT_PROCI_TOUCHSUPPRESSION_T42,0,
		.offset = 0,.buf = {0x1}, .len = 1, .mask = 0x1, .flag = 0, .tag = 0, .sleep = 0},
...
#if defined(CONFIG_DISABLE_KEYS_AT_SUSPEND)
	{.reg = MXT_TOUCH_PTC_KEYS_T97, .instance = 0, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
	{.reg = MXT_TOUCH_PTC_KEYS_T97, .instance = 1, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
	{.reg = MXT_TOUCH_PTC_KEYS_T97, .instance = 2, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
	{.reg = MXT_TOUCH_PTC_KEYS_T97, .instance = 3, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},

//	{.reg = MXT_TOUCH_KEYARRAY_T15,
//		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
#endif
	...
	{.reg = MXT_TOUCH_MULTITOUCHSCREEN_T100,
		.offset = 1,.buf = {0x0}, .len = 1, .mask = 0xe0,.flag = BIT_MASK(P_COMMON)},
...
ssize_t plugin_proci_pi_trace_show(struct plugin_proci *p, char *buf, size_t count)
{
	...
	t_buf = obs->trace_buf;
	strcnt = t_buf[GESTURE_TRACE_STROKE_CNT_INDEX] & 0x1F;
	...
	t_buf += 3;
	for (i = 0; i < strcnt && count > 0; i++) {
		offset += scnprintf(buf + offset, count - offset, "%d,%d;\n", 
...
static int plugin_proci_pi_show(struct plugin_proci *p)
{
	...
	dev_info(dev, "[mxt]pi status: Flag=0x%08lx\n",
		obs->flag);
		...
		if (count > 6 && count < sizeof(name)) {
			...
			if (ret == 1) {
				if (strncmp(name, "config", 6) == 0) {
					...
					r = cfg->reg_cfg[PI_BINDING];
					memset(&rc, 0, sizeof(struct reg_config));
					ofs = 0;
					ret = sscanf(buf + offset, "T%hd offset %hd len %hd: %n", 
						&rc.reg, &rc.offset, &rc.len, &ofs);
					if (ret > 0) {
						dev_info(dev, "%s\n",buf + offset);
						dev_dbg(dev, "T%hd offset %hd len %hd:(ret %d)^",rc.reg, rc.offset,rc.len,ret);
						if (rc.len > MAX_REG_DATA_LEN)
							rc.len = MAX_REG_DATA_LEN;
						...
						ret = binding_list_config_set(p, &rc);
						if (ret)
							dev_info(dev, "faild cmd string: %s\n", buf + offset);
					}else{
						dev_info(dev, "invalid string: %s\n", buf + offset);
					}
				}else if (strncmp(name, "select", 6) == 0) {
					r = cfg->reg_cfg[PI_BINDING];
					ret = sscanf(buf + offset, "%d", &i); 
					if (ret > 0) {
						ret = option_list_config_set(p, i);
						if (ret)
							dev_info(dev, "faild cmd string: %s\n", buf + offset);
					}else{
						dev_info(dev, "invalid string: %s\n", buf + offset);
...
static int init_pi_object(struct plugin_proci *p)
{
	...
	cfg->reg_cfg[PI_BINDING] = mxt_binding_list;
	cfg->num_reg_cfg[PI_BINDING] = ARRAY_SIZE(mxt_binding_list);	
	cfg->reg_cfg[PI_OPTION] = mxt_option_cfg;
	cfg->num_reg_cfg[PI_OPTION] = ARRAY_SIZE(mxt_option_cfg);
