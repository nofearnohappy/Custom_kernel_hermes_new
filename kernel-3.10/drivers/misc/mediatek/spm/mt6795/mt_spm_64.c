...
extern void aee_rr_rec_suspend_debug_flag(u32 val);
#endif
/**************************************
 * Define and Declare
 **************************************/
...
static void spm_register_init(void)
{
    ...
    if (!node) {
        spm_err("[DDRPHY0] find node failed\n");
    }	
    spm_ddrphy0_base = of_iomap(node, 0);
    if (!spm_ddrphy0_base)
        spm_err("[DDRPHY0] base failed\n");

    //ddrphy 1
    node = of_find_compatible_node(NULL, NULL, "mediatek,DDRPHY1");
    if (!node) {
        spm_err("[DDRPHY1] find node failed\n");
    }	
    spm_ddrphy1_base = of_iomap(node, 0);
    if (!spm_ddrphy1_base)
        spm_err("[DDRPHY1] base failed\n");

    //pericfg        
    node = of_find_compatible_node(NULL, NULL, "mediatek,PERICFG");
    if (!node) {
        spm_err("[CLK_PERICFG] find node failed\n");
    }
    spm_pericfg_base = of_iomap(node, 0);
    if (!spm_pericfg_base)
        spm_err("[CLK_PERICFG] base failed\n");        
   ...
   node = of_find_compatible_node(NULL, NULL, "mediatek,SCP_CFGREG");
    if(!node)
        spm_err("[MD32] SCP_CFGREG find node failed\n");
    ...
    spm_err("ddrphy0 = %p, ddrphy1 = %p, pericfg = %p\n", spm_ddrphy0_base, spm_ddrphy1_base, spm_pericfg_base);
#endif
	...
	spm_write(SPM_POWER_ON_VAL0, (1 << 26));
...
#ifdef CONFIG_MTK_RAM_CONSOLE	
	aee_rr_rec_suspend_debug_flag((spm_read(MTK_WDT_NONRST_REG2) >> 30) & 0x3);
	spm_write(MTK_WDT_NONRST_REG2, (spm_read(MTK_WDT_NONRST_REG2) & 0x3FFFFFFF));
#endif
