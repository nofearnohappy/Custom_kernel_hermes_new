...
static int tsallts_get_ts1_temp(void)
{
	...
	tsallts_dprintk("[tsallts_get_ts1_temp] T_CPU, %d\n", t_ret);
	return t_ret;
}
static int tsallts_get_ts2_temp(void)
{
	...
	tsallts_dprintk("[tsallts_get_ts2_temp] T_CPU, %d\n", t_ret);
	return t_ret;
}
static int tsallts_get_ts3_temp(void)
{
	...
	tsallts_dprintk("[tsallts_get_ts3_temp] T_CPU, %d\n", t_ret);
	return t_ret;
}
static int tsallts_get_ts4_temp(void)
{
	...
	tsallts_dprintk("[tsallts_get_ts4_temp] T_CPU, %d\n", t_ret);
	return t_ret;
}
			       ...
			       unsigned long *t)
{
	...
	if (curr_temp != 0) // not the first temp read after resume from suspension
	{
		...
		{
			printk("[Power/ALLTS_Thermal] TS1 temp invalid=%d\n", curr_temp);
			temp_temp = 50000;
			ret = -1;
		}
	    else if (last_abb_read_temp != 0)
	    {
	    	...
	    	{
	      		printk("[Power/ALLTS_Thermal] TS1 temp float hugely temp=%d, lasttemp=%d\n", curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
	       	}
    	}
	...
	*t = (unsigned long) curr_temp;
	...
	int curr_temp;
	curr_temp = tsallts_get_ts1_temp();
    tsallts_dprintk(" tsallts_get_temp CPU T2=%d\n", curr_temp);
			       ...
			       unsigned long *t)
{
    ...
    tsallts_dprintk("tsallts_get_temp2 CPU T2=%d\n", curr_temp);

    if ((curr_temp > (trip_temp[0] - 15000)) || (curr_temp < -30000) || (curr_temp > 85000)) // abnormal high temp
        printk("[Power/ALLTS_Thermal] TS2 T=%d\n", curr_temp);
	...
	if (curr_temp != 0) // not the first temp read after resume from suspension
	{
		if ((curr_temp > 150000) || (curr_temp < -20000)) // invalid range
		{
			printk("[Power/ALLTS_Thermal] TS2 temp invalid=%d\n", curr_temp);
			temp_temp = 50000;
			ret = -1;
		}
	    else if (last_abb_read_temp != 0)
	    {
	    	if ((curr_temp - last_abb_read_temp > 20000) ||(last_abb_read_temp - curr_temp > 20000)) //delta 20C, invalid change
	    	{
	      		printk("[Power/ALLTS_Thermal] TS2 temp float hugely temp=%d, lasttemp=%d\n", curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
	       	}
    	}
			       ...
			       unsigned long *t)
{
    ...
    tsallts_dprintk("tsallts_get_temp3 CPU T2=%d\n", curr_temp);

    if ((curr_temp > (trip_temp[0] - 15000)) || (curr_temp < -30000) || (curr_temp > 85000)) // abnormal high temp
        printk("[Power/ALLTS_Thermal] TS3 T=%d\n", curr_temp);
	...
	if (curr_temp != 0) // not the first temp read after resume from suspension
	{
		if ((curr_temp > 150000) || (curr_temp < -20000)) // invalid range
		{
			printk("[Power/ALLTS_Thermal] TS3 temp invalid=%d\n", curr_temp);
			temp_temp = 50000;
			ret = -1;
		}
	    else if (last_abb_read_temp != 0)
	    {
	    	if ((curr_temp - last_abb_read_temp > 20000) ||(last_abb_read_temp - curr_temp > 20000)) //delta 20C, invalid change
	    	{
	      		printk("[Power/ALLTS_Thermal] TS3 temp float hugely temp=%d, lasttemp=%d\n", curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
	       	}
    	}
			       ...
			       unsigned long *t)
{
    ...
    tsallts_dprintk("tsallts_get_temp4 CPU TS4=%d\n", curr_temp);

    if ((curr_temp > (trip_temp[0] - 15000)) || (curr_temp < -30000) || (curr_temp > 85000)) // abnormal high temp
        printk("[Power/ALLTS_Thermal] TS4 T=%d\n", curr_temp);
	...
	if (curr_temp != 0) // not the first temp read after resume from suspension
	{
		if ((curr_temp > 150000) || (curr_temp < -20000)) // invalid range
		{
			printk("[Power/ALLTS_Thermal] TS4 temp invalid=%d\n", curr_temp);
			temp_temp = 50000;
			ret = -1;
		}
	    else if (last_abb_read_temp != 0)
	    {
	    	if ((curr_temp - last_abb_read_temp > 20000) ||(last_abb_read_temp - curr_temp > 20000)) //delta 20C, invalid change
	    	{
	      		printk("[Power/ALLTS_Thermal] TS4 temp float hugely temp=%d, lasttemp=%d\n", curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
	       	}
    	}
				 ...
				 unsigned long state)
{
    ...
    if(cl_dev_sysrst_state == 1)
	{
		...
		printk("Power/ALL_Thermal: reset, reset, reset!!!");
		printk("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		printk("*****************************************");
		printk("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
static ssize_t tsallts_write2(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
		...
		&time_msec) == 32)
	{
		...
		g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
			g_THERMAL_TRIP[0],g_THERMAL_TRIP[1],g_THERMAL_TRIP[2],g_THERMAL_TRIP[3],g_THERMAL_TRIP[4],
			g_THERMAL_TRIP[5],g_THERMAL_TRIP[6],g_THERMAL_TRIP[7],g_THERMAL_TRIP[8],g_THERMAL_TRIP[9]);
		tsallts_dprintk("[tsallts_write2] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,cooldev4=%s,\
		cooldev5=%s,cooldev6=%s,cooldev7=%s,cooldev8=%s,cooldev9=%s\n",
			g_bind0,g_bind1,g_bind2,g_bind3,g_bind4,g_bind5,g_bind6,g_bind7,g_bind8,g_bind9);
...
static ssize_t tsallts_write3(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	tsallts_dprintk("[tsallts_write3] \n");
...
static ssize_t tsallts_write4(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	tsallts_dprintk("[tsallts_write4] \n");
