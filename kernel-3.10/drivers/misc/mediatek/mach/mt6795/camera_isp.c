...
static MINT32 ISP_DumpReg(void)
{
    ...
    /*  */
    LOG_DBG("0x%08X %08X ", (unsigned int)IMGSYS_BASE_ADDR, (unsigned int)ISP_RD32(ISP_ADDR_CAMINF));
    ...
    LOG_DBG("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D10), (unsigned int)ISP_RD32(ISP_ADDR + 0xD10));
    LOG_DBG("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D20), (unsigned int)ISP_RD32(ISP_ADDR + 0xD20));
    /* cq */
    LOG_DBG("CQ Related");
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x6000);
    LOG_DBG("0x%08X %08X (0x15004160=6000)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x7000);
    LOG_DBG("0x%08X %08X (0x15004160=7000)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x8000);
    LOG_DBG("0x%08X %08X (0x15004160=8000)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    /* imgi_debug */
    LOG_DBG("IMGI_DEBUG Related");
    ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x001e);
    LOG_DBG("0x%08X %08X (0x150075f4=001e)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x011e);
    LOG_DBG("0x%08X %08X (0x150075f4=011e)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x021e);
    LOG_DBG("0x%08X %08X (0x150075f4=021e)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x031e);
    LOG_DBG("0x%08X %08X (0x150075f4=031e)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    /* yuv */
    LOG_DBG("yuv-mdp crop Related");
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x3014);
    LOG_DBG("0x%08X %08X (0x15004160=3014)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    LOG_DBG("yuv-c24b out Related");
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x301e);
    LOG_DBG("0x%08X %08X (0x15004160=301e)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x301f);
    LOG_DBG("0x%08X %08X (0x15004160=301f)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x3020);
    LOG_DBG("0x%08X %08X (0x15004160=3020)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
    ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x3021);
    LOG_DBG("0x%08X %08X (0x15004160=3021)", (unsigned int)(ISP_TPIPE_ADDR + 0x4164), (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
...
#if 1 /* _mt6593fpga_dvt_use_ */
{
    ...
    unsigned long map_va = 0, map_size;
    int i;
    int *pMapVa;
#define TPIPE_DUMP_SIZE    200

    LOG_DBG("kk:tpipePA(0x%x),ctlStart(0x%x),ctlTcm(0x%x)",tpipePA,ctlStart,ctlTcm);
    ...
    MUINT32         Count)
{
	...
	{
	    ISP_WR32((unsigned long)(ISP_ADDR_CAMINF + pReg[i].Addr), pReg[i].Val);
	}
	else
	{
...
/* case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE: */
			...
			if (_bFlag == MTRUE) {
				...
				if (copy_from_user(&deque_buf.data[0], (void __user *)rt_buf_ctrl.ex_data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT)) == 0) {
					...
					/*  */
					{
						...
						spin_unlock_irqrestore(&(IspInfo.SpinLockIrq[irqT_Lock]), flags);
						IRQ_LOG_PRINTER(irqT, 0, _LOG_DBG);
						LOG_ERR("[rtbc]dma(%d),PA(0x%x),over enque", rt_dma, pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr);
						return -EFAULT;
					    }
				...
				for (i = 0; i < ISP_RT_BUF_SIZE; i++) {
					...
					if (pstRTBuf->ring_buf[rt_dma].empty_count < pstRTBuf->ring_buf[rt_dma].total_count)
					    ...
					    spin_unlock_irqrestore(&(IspInfo.SpinLockIrq[irqT_Lock]), flags);
					    IRQ_LOG_PRINTER(irqT, 0, _LOG_DBG);
					    LOG_ERR("[rtbc]error:dma(%d),PA(0x%x),over enque", rt_dma, pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr);
					    return -EFAULT;
					}
				...
				LOG_DBG("[rtbc][ENQUE]:dma(%d),PA(0x%x),O(0x%x),ZO(0x%x),O_D(0x%x),ZO_D(0x%x),camsv(0x%x/0x%x)fps(%d/%d/%d/%d)us,IMD_%d\n",
				    rt_dma, \
		...
		if ((reg_val & 0x01) || (reg_val2 & 0x01) || (camsv_reg_cal[0] & 0x01) || (camsv_reg_cal[1] & 0x01)) {
			...
			for (i = 0; i < deque_buf.count; i++) {
			    ...
			    bBufFilled = MTRUE;
			    if (pstRTBuf->ring_buf[rt_dma].data[iBuf+i].bFilled != ISP_RTBC_BUF_FILLED) {
				bBufFilled = MFALSE;
			    }
			    ...
			    if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
				LOG_DBG("[rtbc][DEQUE](%d):d(%d)/id(0x%x)/bs(0x%x)/va(0x%llx)/pa(0x%x)/t(%d.%d)/img(%d,%d,%d,%d,%d,%d,%d,%d)/m(0x%x)/fc(%d)/rrz(%d,%d,%d,%d,%d,%d),lm#(0x%x)", \
				    iBuf+i,\
				    rt_dma, \
			    ...
			    /*  */
#if 0
				LOG_DBG("[rtbc][DEQUE]:D(%d),TStamp\"%d.%06d\",o(0x%08x),zo(0x%08x),i(%d),VA(0x%x),PA(0x%x),O(0x%x),2O(0x%x)", \
				    rt_dma, \
				    deque_buf.data[i].timeStampS, \
				    deque_buf.data[i].timeStampUs, \
				    p1_fbc[_imgo_], \
				...
				LOG_DBG("[rtbc][DEQUE]:o_d(0x%08x),zo_d(0x%08x),O_D(0x%x),2O_D(0x%x)", \
				    p1_fbc[_imgo_d_], \
				    p1_fbc[_rrzo_d_], \
				    ISP_RD32(ISP_REG_ADDR_IMGO_D_BASE_ADDR), \
				    ISP_RD32(ISP_REG_ADDR_RRZO_D_BASE_ADDR));
				LOG_DBG("[rtbc][DEQUE]:camsv(0x%x/0x%x),camsv_imgo(0x%x/0x%x)", \
				    p1_fbc[_camsv_imgo_], \
...
LOG_DBG("+LARB in DEQUE,BWL(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X),220(0x%08X)/(0x%08X),0x14(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)", \
    ...
    ISP_RD32(SMI_LARB4+0x10));
		...
		switch (rt_dma) {
		    case _imgo_:
		    case _rrzo_:
			/* reset timestamp record for wait_irq */
...
static MINT32 ISP_SOF_Buf_Get(eISPIrq irqT, unsigned long long sec, unsigned long usec, MBOOL bDrop, CQ_RTBC_FBC * pFbc, MUINT32 *pCurr_pa)
{
#if defined(_rtbc_use_cq0c_)
...
/* above behavior will make FBC write-buffer-patch fail at p1_done */
/* curr_pa also have this prob. too */
static MINT32 ISP_CAMSV_SOF_Buf_Get(unsigned int dma, unsigned long long sec, unsigned long usec, MBOOL bDrop, CQ_RTBC_FBC camsv_fbc, MUINT32 curr_pa)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_INT_2) {
	   IRQ_LOG_KEEPER(irqT, m_CurrentPPB, _LOG_INF, "sv%d T(%d.%06d),cur(%d),addr(0x%x),prv(0x%x),fbc(0x%08x)\n", \
		...
		PrvAddr[out], \
		(unsigned int)camsv_fbc.Reg_val);
	}
	/*  */
    }
...
/* above behavior will make FBC write-buffer-patch fail at p1_done */
/* curr_pa also have this prob. too */
static MINT32 ISP_CAMSV_DONE_Buf_Time(unsigned int dma, unsigned long long sec, unsigned long usec, CQ_RTBC_FBC fbc)
{
    unsigned int curr;
...
/* mark the behavior of reading FBC at local. to prevent hw interruptting duing sw isr flow. */
/* above behavior will make FBC write-buffer-patch fail at p1_done */
static MINT32 ISP_DONE_Buf_Time(eISPIrq irqT, unsigned long long sec, unsigned long usec, CQ_RTBC_FBC *pFbc)
{
    int i, k;
    int i_dma;
    unsigned int curr;
	...
	for (i = 0; i <= 1; i++) {
		...
		else
		{
			...
			LOG_ERR("dma_%d,cur_%d,bFilled_%d != EMPTY(%d %d %d)\n",\
				i_dma,\
				curr,\
				pstRTBuf->ring_buf[i_dma].data[curr].bFilled,\
				pstRTBuf->ring_buf[i_dma].data[0].bFilled, pstRTBuf->ring_buf[i_dma].data[1].bFilled, pstRTBuf->ring_buf[i_dma].data[2].bFilled);
...
static MINT32 ISP_RegisterIrq(ISP_WAIT_IRQ_STRUCT RegisterIrq)
{
    ...
    MUINT32 flags;
    MUINT32 i = 0;
    MINT32 idx = IspInfo.IrqInfo.WaitQIdx[(RegisterIrq.Type*32)+my_get_pow_idx(RegisterIrq.Status)];
    if (idx < 0)
    {
...
static MINT32 ISP_UnRegisterIrq(ISP_WAIT_IRQ_STRUCT RegisterIrq)
{
	...
	{   /* traverse user child nodes */
		...
		ptr->nextuser = NULL;
		LOG_INF("free one,user(%s),reg_T(%d),Name(%s)\n", accessNode->userName, accessNode->reg_T, accessNode->userName);
		kfree(accessNode);
		break;
	    }
	    else
	...
	ptr = ptr->nextuser;
	accessNode = list_entry(ptr, REG_IRQ_NODE, list);
	LOG_INF("cnt(%d) name(%s) reg_T(%d)\n", ++cnt, accessNode->userName, accessNode->reg_T);
    }
    #endif
    LOG_INF("-\n");
...
static MINT32 ISP_UnRegister_AllregIrq()
{
	...
	{
	    userchild = father;
	    if (userchild->nextuser != NULL)
	    {/* traverse and delete user child nodes */
...
static MINT32 ISP_WaitIrq(ISP_WAIT_IRQ_STRUCT * WaitIrq)
{
    ...
    {
	spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
	LOG_ERR("WaitIrq Timeout User(%d), Clear(%d), Type(%d), IrqStatus(0x%08X), WaitStatus(0x%08X), Timeout(%d)",
                    WaitIrq->UserNumber,
				    WaitIrq->Clear,
				    WaitIrq->Type,
    ...
    void *DeviceId)
{
    ...
    {
	IspInfo.IrqInfo.Status[j][ISP_IRQ_TYPE_INT_CAMSV] |= (IrqStatus_CAMSV & IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INT_CAMSV]);
    }
    ...
    if (IspInfo.DebugMask & ISP_DBG_INT) {
	IRQ_LOG_KEEPER(_CAMSV_IRQ, m_CurrentPPB, _LOG_INF, CAMSV_TAG"Type(%d), IrqStatus(0x%p | 0x%x)\n", ISP_IRQ_TYPE_INT_CAMSV, IspInfo.IrqInfo.Status[ISP_IRQ_TYPE_INT_CAMSV], (IrqStatus_CAMSV));
    }
	...
	}
	{
	    ...
	    ISP_CAMSV_SOF_Buf_Get(_camsv_imgo_, sec, usec, gSof_camsvdone[0], fbc, curr_pa);
	}
    }
    ...
    void *DeviceId)
{
    ...
    for (j = 0; j < ISP_IRQ_USER_MAX; j++)
    {
	IspInfo.IrqInfo.Status[j][ISP_IRQ_TYPE_INT_CAMSV2] |= (IrqStatus_CAMSV2 & IspInfo.IrqInfo.Mask[ISP_IRQ_TYPE_INT_CAMSV2]);
    }
    ...
    if (IspInfo.DebugMask & ISP_DBG_INT)
    {
	IRQ_LOG_KEEPER(_CAMSV_D_IRQ, m_CurrentPPB, _LOG_INF, CAMSV2_TAG"Type(%d), IrqStatus(0x%p | 0x%08x)\n", ISP_IRQ_TYPE_INT_CAMSV2, IspInfo.IrqInfo.Status[ISP_IRQ_TYPE_INT_CAMSV2], (IrqStatus_CAMSV2));
    }
	...
	}
	{
	    ...
	    ISP_CAMSV_SOF_Buf_Get(_camsv2_imgo_, sec, usec, gSof_camsvdone[1], fbc, curr_pa);
	}
    }
...
/* LOG_DBG("- E."); */
	...
	usec = do_div(sec, 1000000);    /* sec and usec */
	/* update pass1 done time stamp for eis user(need match with the time stamp in image header) */
	IspInfo.IrqInfo.LastestSigTime_usec[ISP_IRQ_TYPE_INT_P1_ST_D][10] = (unsigned int)(sec);
	IspInfo.IrqInfo.LastestSigTime_sec[ISP_IRQ_TYPE_INT_P1_ST_D][10] = (unsigned int) (usec);
           gEismetaInSOF_D = 0;
    ...
    unsigned long   Param)
{
	...
	case ISP_GET_INT_ERR:
	    if (copy_to_user((void *)Param, (const void *)g_ISPIntErr, sizeof(MUINT32)*_IRQ_MAX) != 0) {
	...
	case ISP_WAIT_IRQ:
	{
		...
		if (copy_to_user((void *)Param, &WaitIrq, sizeof(ISP_WAIT_IRQ_STRUCT)) != 0)
		{
    ...
    void *pData)
{
    ...
    {
	ISP_WR32((unsigned long)(ISP_ADDR_CAMINF + Addr), Data);
    ...
    void *pData)
{
    ...
    /*  */
    p += sprintf(p, "reg_0x%x = 0x%08x\n", (int)(ISP_ADDR_CAMINF+proc_regOfst) , (int)(ioread32(ISP_ADDR_CAMINF + proc_regOfst)));
...
#define PER_LINE_LOG_SIZE   10
    ...
    for (i = 0; i < regCount; i += PER_LINE_LOG_SIZE) {
    	...
    	LOG_INF("[0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x]\n", \
    	    add[0], val[0], add[1], val[1], add[2], val[2], add[3], val[3], add[4], val[4], add[5], val[5], add[6], val[6], add[7], val[7], add[8], val[8], add[9], val[9]);
    }
