...
void __iomem  *clk_e3tcm_base;
//void __iomem  *clk_mcu_cfg_base;
void __iomem  *clk_rgu_base;
#endif
...
#endif
    clk_writel(clk_rgu_base+0x54, 0x80000000);//use weiqi API later
...
//    clk_writel(DISP_CG_CLR1, DISP1_CG);
    ...
    //E3TCM
    //clk_writel(E3TCM_CG_CLR, E3TCM_CG);
}
...
//    clk_writel(DISP_CG_SET1, 0xc);
    ...
    //E3TCM
    //clk_writel(E3TCM_CG_SET, E3TCM_CG);
}
...
int mt_clkmgr_init(void)
{
    ...
    initialized = 1;
    mt_freqhopping_init(); 
    //mt_freqhopping_pll_init(); //already in hopping driver
    print_grp_regs();
...
*	hf_fmm_ck:  VENCPLL_D2(400MHz)   <-->VENCPLL_D2(317MHz), signoff:317MHz,E3
*	hf_fvenc_ck: VCODECPLL_CK(494MHz)<-->VCODECPLL_CK(324MHz), signoff:324MHz, E3
*	hf_fvdec_ck: VCODECPLL_CK(494MHz)<-->VCODECPLL_CK(324MHz), signoff:420MHz, E3
...
void clkmux_sel_for_vcorefs(bool mode)
{
    ...
    else
    {
        ...
        mt_fh_hal_dfs_vcodecpll(0xC7627); //VCODECPLL=324M
#endif
        pll_disable_internal(pll, "clkmgr");
...
/*************E3TCM****************/
void e3tcm_pwr_ctrl(int state)
{
	unsigned long flags;
	clkmgr_lock(flags);
	
    if (state == STA_POWER_DOWN) {
        spm_ctrl_e3tcm(STA_POWER_DOWN);	
        clk_writel(E3TCM_CG_SET, 0x7F);
    } else {
        clk_writel(E3TCM_CG_CLR, 0x7F);
        spm_ctrl_e3tcm(STA_POWER_ON);	
    }
