...
static int ram_console2_show(struct seq_file *m, void *v)
{
	...
	if (bufp->off_console < ram_console_buffer_size)
		seq_printf(m, "%s", (char*)bufp + bufp->off_console);
	return 0;
}
...
{
	seq_printf(m, " fiq step: %u\n", LAST_RRR_VAL(fiq_step));
}
