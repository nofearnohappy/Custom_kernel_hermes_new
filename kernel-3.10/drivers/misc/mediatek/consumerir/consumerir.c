...
#ifdef GPIO_IRTX_OUT_PIN
pr_warning("[IRTX] open GPIO_IRTX_OUT_PIN defined\n");
#else 
    #ifdef GPIO_ANT_SEL0
        pr_warning("[IRTX] open GPIO_ANT_SEL0 defined\n");
    #else
        pr_warning("[IRTX] open GPIO19 defined\n");
    #endif
#endif
    ...
    //enable_clock(MT_CG_PERI_IRTX, "IRTX"); //я не знаю где эти значения
    pr_warning("[IRTX] open by %s\n", current->comm);
...
static int dev_char_close(struct inode *inode, struct file *file)
{
    ...
    //disable_clock(MT_CG_PERI_IRTX, "IRTX"); //я не знаю где эти значения
    return 0;
}
    ...
    switch(cmd) {
        case IRTX_IOC_SET_CARRIER_FREQ:
            if(copy_from_user(&mt_irtx_dev.carrier_freq, (void __user *)arg, sizeof(unsigned int))) {
                ...
                pr_warning("[IRTX] IRTX_IOC_SET_CARRIER_FREQ: %d\n", mt_irtx_dev.carrier_freq);
        ...
        case IRTX_IOC_SET_IRTX_LED_EN:
            if(copy_from_user(&para, (void __user *)arg, sizeof(unsigned int))) {
                ...
                en = (para & 0xF);
                pr_warning("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: 0x%x, gpio_id:%ul, en:%ul\n", para, gpio_id, en);
                if (en) {
                    mode = GPIO_MODE_02;
                    dir = GPIO_DIR_OUT;
                ...
                pr_warning("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: gpio:0x%xl, mode:%d\n", gpio_id, mt_get_gpio_mode(gpio_id));
                pr_warning("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: gpio:0x%xl, dir:%d\n", gpio_id, mt_get_gpio_dir(gpio_id));
                pr_warning("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: gpio:0x%xl, out:%d\n", gpio_id, mt_get_gpio_out(gpio_id));
            }
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    set_consumerir_pwm(); //set_irtx_sw_mode
    ...
    pr_warning("[IRTX] pwm is triggered, %d\n", ret);
...
//my edit start
#define irtx_driver_name "consumerir"
static int consumerir_probe(struct platform_device *plat_dev)
{
    ...
    pr_warning("[IRTX] device tree info: major=%d base=0x%p irq=%d pwm=%d\n", 
    ...
    // ret = request_irq(mt_irtx_dev.irq, irtx_isr, IRQF_TRIGGER_FALLING, "IRTX", NULL); // TODO: trigge
    ret = request_irq(mt_irtx_dev.irq, consumerir_isr, IRQF_TRIGGER_FALLING, "IRTX", NULL); // TODO: trigger
    if(ret) {
        pr_err("[IRTX] request IRQ(%d) fail ret=%d\n", mt_irtx_dev.irq, ret);
        goto exit;
    }
    //irtx_pwm_config.pwm_no = mt_irtx_dev.pwm_ch;
    consumerir_pwm_config.pwm_no = mt_irtx_dev.pwm_ch;
