...
#ifdef BUILD_LK
#define LCD_LOG(fmt, ...)  do { dprintf(ALWAYS, fmt, ##__VA_ARGS__); } while(0)
#else
#define LCD_LOG(fmt, ...)  do { printk(fmt, ##__VA_ARGS__); } while(0)
#endif
...
typedef struct
{
    unsigned int mipi_pll_clk_ref;   
    unsigned int mipi_pll_clk_div1;  
    unsigned int mipi_pll_clk_div2;  
    unsigned int mipi_pll_clk_fbk_div;  
    unsigned int dpi_clk_div;        
    unsigned int dpi_clk_duty;       
...
typedef struct
{
	...
	void (*dsi_set_cmdq_V23)(void* cmdq, unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update);
} LCM_UTIL_FUNCS;
typedef enum
{
