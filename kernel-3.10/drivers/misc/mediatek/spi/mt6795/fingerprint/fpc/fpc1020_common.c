...
struct mt_chip_conf spi_conf_mt65xx = {
        .setuptime = 15,
        .holdtime = 15,
        .high_time = 10, //for mt6589, 100000khz/(4+4) = 125000khz
        .low_time = 10,
        .cs_idletime = 20,
        .ulthgh_thrsh = 0,
...
int fpc1020_reset(fpc1020_data_t *fpc1020)
{
	...
	if (error) {
		dev_err(&fpc1020->spi->dev, "[WYQ]IRQ pin, not low after clear.\n");
...
#ifdef FPC1020_SPI_DEBUG
	cmd[0] = FPC102X_REG_HWID;
	error = fpc1020_spi_wr_rd(fpc1020, cmd, cmd+3, 4);
	for (i = 0; i < 8; i++)
		printk("[WYQ] fpc1020_check_hw_id cmd[%d]=0x%x\n", i, cmd[i]);
#endif
	if (error)
		return error;
...
#ifdef FPC1020_SPI_DEBUG
	hardware_id= cmd[4]<<8;
	hardware_id = hardware_id + cmd[5];
#endif
	printk("[WYQ] hw id=0x%x\n", hardware_id);
...
static void mtspi_set_mode(fpc1020_data_t *fpc1020, int mode)
{
        ...
        if(!spi_par)
        {
                dev_err(&fpc1020->spi->dev, "spi config fail\n");
                return;
        }
        ...
        {
                dev_err(&fpc1020->spi->dev, "spi_setup  fail\n");
        }
...
static int fpc1020_write_sensor_1021_setup(fpc1020_data_t *fpc1020) 
{
	...
	fpc1020_reg_access_t reg_1 = {
                .reg = FPC102X_REG_FINGER_DRIVE_DLY,
                .write = false,
                .reg_size = FPC1020_REG_SIZE(FPC102X_REG_FINGER_DRIVE_DLY),
                .dataptr = &temp_u8
        };
	error = fpc1020_reg_access(fpc1020, &reg_1);
	printk("[WYQ] fpc1020_write_sensor_1021_setup FPC102X_REG_FINGER_DRIVE_DLY read out=0x%x\n", temp_u8);
#endif
...
#if 0
	 fpc1020_reg_access_t reg_2  = {
                .reg = FPC102X_REG_SAMPLE_PX_DLY,
                .write = false,
                .reg_size = FPC1020_REG_SIZE(FPC102X_REG_SAMPLE_PX_DLY),
                .dataptr = &temp_u64
        };      
        error = fpc1020_reg_access(fpc1020, &reg_2);
        printk("[WYQ] fpc1020_write_sensor_1021_setup FPC102X_REG_SAMPLE_PX_DLY read out=0x%llx/%lld\n", temp_u64,temp_u64);
#endif
	temp_u8 = (fpc1020->vddtx_mv > 0) ? 0x02 :	/* external supply */
		(fpc1020->txout_boost) ? 0x22 : 0x12;	/* internal supply */
	FPC1020_MK_REG_WRITE(reg, FPC102X_REG_FINGER_DRIVE_CONF, &temp_u8);
...
#ifdef FPC1020_SPI_DEBUG
	cmd[0] = FPC102X_REG_READ_INTERRUPT_WITH_CLEAR;
	error = fpc1020_spi_wr_rd(fpc1020, cmd, cmd+2, 2);
	for (i =0; i < 8; i++)
		printk("[WYQ]  check irq after reset fpc1020_spi_wr_rd=0x%x\n", cmd[i]);
#endif
...
int fpc1020_read_irq(fpc1020_data_t *fpc1020, bool clear_irq)
{
...
#endif
	//printk("[WYQ] fpc1020_read_irq status =0x%x\n", irq_status);
#if 1	
	if (clear_irq)
		cmd[0] = FPC102X_REG_READ_INTERRUPT_WITH_CLEAR;
	else
		cmd[0] = FPC102X_REG_READ_INTERRUPT;
        error = fpc1020_spi_wr_rd(fpc1020, cmd, cmd+2, 2);
        //for (i =0; i < 8; i++)
        //        printk("[WYQ] fpc1020_read_irq=0x%x\n", cmd[i]);
	...
	if (irq_status == FPC_1020_IRQ_REG_BITS_REBOOT) {
		...
		dev_err(&fpc1020->spi->dev,
			"%s: fpc1020_read_irq unexpected irq_status = 0x%x\n"
			, __func__, irq_status);
...
int fpc1020_read_status_reg(fpc1020_data_t *fpc1020)
{
	...
	printk("[WYQ] fpc1020_read_status_reg status=0x%x\n", status);
//	cmd[0] = FPC102X_REG_FPC_STATUS;
//	error = fpc1020_spi_wr_rd(fpc1020, cmd, cmd+2, 2);
//	for(i = 0; i < 8; i++)
//		printk("[WYQ] fpc1020_read_status_reg status[%d]= 0x%x\n", i, cmd[i]);
		      ...
		      fpc1020_reg_access_t *reg_data)
{
...
#ifdef FPC1020_SPIDEV_DEBUG
                        printk("[WYQ] reg_size:%d,read addr :%d = 0x%x\n", reg_data->reg_size, i, temp_buffer[i]);
#endif
		}
	}
	if (reg_data->write) {
		for(i = 0; i < (1+FPC1020_REG_ACCESS_DUMMY_BYTES(reg_data->reg)); i ++) {
			...
			temp_buffer[i] = (reg_data->reg>>shift)&0xFF;
#ifdef FPC1020_SPIDEV_DEBUG
			printk("[WYQ] reg_size:%d,write_cmd:%d = 0x%x\n", reg_data->reg_size, i, temp_buffer[i]);
#endif
...
#ifdef FPC1020_SPIDEV_DEBUG
		for(i = 0; i < reg_data->reg_size; i++)
                        printk("[WYQ] reg_size:%d,write_data:%d = 0x%x\n", reg_data->reg_size, i, reg_data->dataptr[i]);
#endif
...
#ifdef FPC1020_SPIDEV_DEBUG
		for(i = 0; i < reg_data->reg_size + 1+FPC1020_REG_ACCESS_DUMMY_BYTES(reg_data->reg); i ++)
		printk("[WYQ] reg_size:%d,write_cmd_data:%d = 0x%x\n", reg_data->reg_size, i, temp_buffer[i]);
#endif
	}
	...
	if (!reg_data->write) {
		...
		int dst = 0;
#ifdef FPC1020_SPIDEV_DEBUG
		for(i = 0; i < reg_data->reg_size<<1; i++)
			printk("[WYQ] reg_size:%d,readout data :%d = 0x%x\n", reg_data->reg_size, i, temp_buffer2[i]);
#endif
...
#ifdef FPC1020_SPIDEV_DEBUG
		for(i = 0; i < reg_data->reg_size; i++)
                        printk("[WYQ] reg_size:%d,readout_user data :%d = 0x%x\n", reg_data->reg_size, i, reg_data->dataptr[i]);
#endif
...
#if 0
	printk("[WYQ]%s %s 0x%x/%dd (%d bytes) %x %x %x %x : %x %x %x %x\n",
		 __func__,
	...
	if (reset == 0 && status >= 0 &&
		...
		return 0;
		 ...
		 fpc1020_reg_access_t reg_1 = {
                ...
                printk("[WYQ]  fpc1020_wake_up FpcError reg=0x%x\n", temp_u8);
				...
				size_t buff_size)
{
...
#endif
		error = fpc1020_read_irq(fpc1020, true);
		printk("[WYQ] fpc1020_fetch_image 0#irq_status=%d\n", error);
	...
	error = fpc1020_read_irq(fpc1020, true);
	printk("[WYQ] fpc1020_fetch_image 1#irq_status=%d\n", error);
	if (error > 0)
		error = (error & FPC_1020_IRQ_REG_BIT_ERROR) ? -EIO : 0;

	printk("[WYQ] fpc1020_fetch_image return error=%d\n", error);
	return error;
}
...
int fpc1020_calc_finger_detect_threshold_min(fpc1020_data_t *fpc1020)
{
	...
	//image_size = (FPC1020_WAKEUP_DETECT_ROWS) * adc_groups * fpc1020->chip.adc_group_size;
        //row_count = FPC1020_WAKEUP_DETECT_ROWS;
	...
	while (index && !error) {
		...
		//first_row = fpc1020->setup.wakeup_detect_rows[index];
		error = fpc1020_capture_set_crop(fpc1020,
						first_col,
						adc_groups,
						first_row,
		...
		if (!error) {
			error = fpc1020_capture_buffer(fpc1020,
						...
						0,
						image_size);
			printk("[WYQ] fpc1020_calc_finger_detect_threshold_min error=%d, LINE =%d\n", error, __LINE__);
		}
