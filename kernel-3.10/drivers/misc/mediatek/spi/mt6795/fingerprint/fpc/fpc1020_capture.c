...
int fpc1020_capture_task(fpc1020_data_t *fpc1020)
{
	...
	if (error < 0)
		goto out_error;
	printk("[WYQ] fpc1020_capture_task mode = %d\n", mode);
	switch (mode) {
	...
	if (error < 0) {
		printk("[WYQ] fpc1020_capture_task line=%d, error=%d\n",__LINE__,  error);
		goto out_error;
	}
...
int fpc1020_capture_wait_finger_down(fpc1020_data_t *fpc1020)
{
	...
	printk("[WYQ] fpc1020_capture_wait_finger_down =%d\n", finger_down);
...
int fpc1020_capture_wait_finger_up(fpc1020_data_t *fpc1020)
{
	...
	printk("[WYQ] fpc1020_capture_wait_finger_up =%d\n", finger_up);
					...
					int num_rows)
{
	...
	printk("[WYQ] fpc1020_capture_set_crop temp_u32=%d\n", temp_u32);
	FPC1020_MK_REG_WRITE(reg,FPC102X_REG_IMG_CAPT_SIZE, &temp_u32);
	ret = fpc1020_reg_access(fpc1020, &reg);

#if 0
	fpc1020_reg_access_t reg_1 = {
                .reg = FPC102X_REG_IMG_CAPT_SIZE,
                .write = false,
                .reg_size = FPC1020_REG_SIZE(FPC102X_REG_IMG_CAPT_SIZE),
                .dataptr = &temp_u321,
        };
        fpc1020_reg_access(fpc1020, &reg_1);
        printk("[WYQ] fpc1020_capture_set_crop reg,FPC102X_REG_IMG_CAPT_SIZE read out=%d\n", temp_u321);
#endif
	return ret;
}
				...
				size_t image_size_bytes)
{
	...
	if (error < 0) {
		printk("[WYQ] fpc1020_capture_buffer =%d\n", error);
		goto out_error;
	}
	...
	error = fpc1020_fetch_image(fpc1020,
				    ...
				    (size_t)fpc1020->huge_buffer_size);
	if (error < 0) {
		printk("fpc1020_capture_buffer_fpc1020_fetch_image=%d\n", error);
		goto out_error;
	}
