...
#define GPIO_FPC_1V8_EN_PIN  (GPIO135 | 0x80000000)
#define GPIO_FPC_3V3_EN_PIN  (GPIO134 | 0x80000000)

#define GPIO_FPC_SPICK_PIN  (GPIO166 | 0x80000000)
#define GPIO_FPC_SPIMI_PIN  (GPIO167 | 0x80000000)
#define GPIO_FPC_SPIMO_PIN  (GPIO168 | 0x80000000)
#define GPIO_FPC_SPICS_PIN  (GPIO169 | 0x80000000)
...
//		goto err_cdev;
	...
	up(&fpc1020->mutex);
        ...
        mt_eint_unmask(fpc1020->irq);
#endif
	printk("[FPC] fpc1020 probe success\n");
	return 0;
...
{
	int error = 0;
        mt_set_gpio_mode(GPIO_FPC_1V8_EN_PIN,GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_FPC_1V8_EN_PIN,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_FPC_1V8_EN_PIN,GPIO_OUT_ONE);

        mt_set_gpio_mode(GPIO_FPC_3V3_EN_PIN,GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_FPC_3V3_EN_PIN,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_FPC_3V3_EN_PIN,GPIO_OUT_ONE);
#if 1
        mt_set_gpio_mode(GPIO_FPC_SPICK_PIN,GPIO_MODE_01);

        mt_set_gpio_mode(GPIO_FPC_SPIMI_PIN,GPIO_MODE_01);

        mt_set_gpio_mode(GPIO_FPC_SPIMO_PIN,GPIO_MODE_01);

        mt_set_gpio_dir(GPIO_FPC_SPICS_PIN,GPIO_MODE_01);
#endif
	return 0;
err:
	return error;
...
static int  fpc1020_create_device(fpc1020_data_t *fpc1020)
{
	...
	if (IS_ERR(fpc1020->device)) {
		printk("[WYQ] fpc1020 device_create failed.\n");
