	...
	if(layer_info->timeline != NULL)
	{
		...
		list_for_each_entry_safe(buf,n,&layer_info->buf_list, list) 
    	{
			...
			if(buf->hnd)
			{
				...
				DISPPR_FENCE("R+/%s%d/L%d/id%d/last%d/new%d/free/idx%d/mva0x%lx\n", disp_session_mode_spy(session_id), 
					DISP_SESSION_DEV(session_id), layer_id, fence, current_timeline_idx, 
					layer_info->fence_idx, buf->idx, buf->mva);
    		}
    	}
			...
			if(buf->idx <= fence_idx)
			{
				...
				if((buf->mva +buf->mva_offset) == phy_addr)
				{
					if(phy_addr >= buf->mva && buf->mva + buf->size > phy_addr){
					    ...
					    DISPPR_ERROR("wrong addr:0x%08lx, 0x%08lx,0x%08x\n", phy_addr, buf->mva, buf->size);
                    }
					idx = buf->idx - 1;					
				}
			}
...
unsigned int disp_sync_query_buf_info(unsigned int session_id, unsigned int timeline_id, unsigned int idx, unsigned long *mva, unsigned int *size) 
{
	...
	if (dst_mva != 0x0) 
	{
		...
		*size = dst_size;
		...
		MTKFB_FENCE_LOG("query buf mva: layer=%u, idx=%du, mva=0x%08x\n", timeline_id, idx, buf->mva);
	} 
	else 
	{
		//FIXME: non-ion buffer need cache sync here?
