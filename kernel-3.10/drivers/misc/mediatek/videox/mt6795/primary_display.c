...
#if CONFIG_FOR_SOURCE_PQ
	// 12.re-build trigger loop
	_cmdq_stop_trigger_loop();
	//_cmdq_build_trigger_loop();
    _cmdq_build_Decouple_trigger_loop();
	_cmdq_start_trigger_loop();
#endif
...
{
	if(!sw_only)
		return _DC_switch_to_DL_fast();
	else
		return _DC_switch_to_DL_sw_only();
}
...
static int _disp_primary_path_check_trigger(void *data)
{
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_USE_CMDQ)) 
	{
		...
		while (1) {
			...
			if (pgc->state != DISP_SLEEPED) {
				primary_display_idlemgr_kick(__func__);
				cmdqRecReset(handle);
...
{
	return __primary_display_switch_mode(sess_mode, session, 1);
}
