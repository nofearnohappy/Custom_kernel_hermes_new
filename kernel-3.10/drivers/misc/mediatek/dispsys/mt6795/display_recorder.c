...
int dprec_logger_pr(unsigned int  type, char *fmt, ...)
{
	...
	buf = dprec_logger_buffer[type][dprec_logger_id[type]] + 16*1024 - dprec_logger_len[type][dprec_logger_id[type]];
	...
	if(buf)
	{
		...
		n += snprintf(buf+n, len-n, "[%5lu.%06lu]", (unsigned long)time, rem_nsec / 1000);
...
char *_logger_pr_type_spy(DPREC_LOGGER_PR_TYPE type)
{
	switch(type)
	{
		case DPREC_LOGGER_ERROR: return "error";
		case DPREC_LOGGER_FENCE: return "fence";
		case DPREC_LOGGER_HWOP: return "hwop";
		default: return "unknown";
	}
}
