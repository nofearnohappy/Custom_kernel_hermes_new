...
#define MIPITX_INREG32(addr)								\		
  	({														\
		...
		if(dsi_reg_op_debug) 								\
		{													\
			\\printk("[mipitx/inreg]0x%08x=0x%08x\n", addr, val);	\
		}													\
		val;													\
	})
...
#define MIPITX_OUTREG32(addr, val) \
  	{\
  		if(dsi_reg_op_debug) \
		{	 } \
		if(0)mt65xx_reg_sync_writel(val, addr);}
	...
	if(level >= 1)
	{
		if(module == DISP_MODULE_DSI0 || module == DISP_MODULE_DSIDUAL)
		{
			...
			if(disp_helper_get_stage() != DISP_HELPER_STAGE_EARLY_PORTING)
			{
			    ...
			    {
			       	DDPDUMP("DSI_PHY+%04x : 0x%08x    0x%08x  0x%08x  0x%08x\n", i, INREG32((DDP_REG_BASE_MIPITX0+i)), INREG32((DDP_REG_BASE_MIPITX0+i+0x4)), INREG32((DDP_REG_BASE_MIPITX0+i+0x8)), INREG32((DDP_REG_BASE_MIPITX0+i+0xc)));
			    }
			}
		}
		...
		if(module == DISP_MODULE_DSI1 || module == DISP_MODULE_DSIDUAL)
		{
			...
			for (i = 0; i < sizeof(DSI_REGS); i += 16)
			{
				DDPDUMP("DSI+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i, INREG32(DDP_REG_BASE_DSI1 + i), INREG32(DDP_REG_BASE_DSI1 + i + 0x4), INREG32(DDP_REG_BASE_DSI1 + i + 0x8), INREG32(DDP_REG_BASE_DSI1 + i + 0xc));
			}

			for (i = 0; i < sizeof(DSI_CMDQ_REGS); i += 16)
			{
				DDPDUMP("DSI_CMD+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i, INREG32((DDP_REG_BASE_DSI1+0x200+i)), INREG32((DDP_REG_BASE_DSI1+0x200+i+0x4)), INREG32((DDP_REG_BASE_DSI1+0x200+i+0x8)), INREG32((DDP_REG_BASE_DSI1+0x200+i+0xc)));
			}
			...
			if(disp_helper_get_stage() != DISP_HELPER_STAGE_EARLY_PORTING)
	    	{
		    	for (i = 0; i < sizeof(DSI_PHY_REGS); i += 16)
			    {
			       	DDPDUMP("DSI_PHY+%04x : 0x%08x    0x%08x  0x%08x  0x%08x\n", i, INREG32((DDP_REG_BASE_MIPITX1+i)), INREG32((DDP_REG_BASE_MIPITX1+i+0x4)), INREG32((DDP_REG_BASE_MIPITX1+i+0x8)), INREG32((DDP_REG_BASE_MIPITX1+i+0xc)));
			    }
			}
		}			
	}
...
//	unsigned int fmod = 30;//Fmod = 30KHz by default
	...
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
		...
		if(dsi_params->lane_swap_en)
		{
			...
			MIPITX_OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG[i]->MIPITX_DSI_PHY_SEL, MIPI_TX_PHY0_SEL,dsi_params->lane_swap[i][MIPITX_PHY_LANE_0]);
			MIPITX_OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG[i]->MIPITX_DSI_PHY_SEL, MIPI_TX_PHY1_SEL,dsi_params->lane_swap[i][MIPITX_PHY_LANE_1]);
			MIPITX_OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG[i]->MIPITX_DSI_PHY_SEL, MIPI_TX_PHY2_SEL,dsi_params->lane_swap[i][MIPITX_PHY_LANE_2]);
			MIPITX_OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG[i]->MIPITX_DSI_PHY_SEL, MIPI_TX_PHY3_SEL,dsi_params->lane_swap[i][MIPITX_PHY_LANE_3]);
			MIPITX_OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG[i]->MIPITX_DSI_PHY_SEL, MIPI_TX_PHYC_SEL,dsi_params->lane_swap[i][MIPITX_PHY_LANE_CK]);
			MIPITX_OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG[i]->MIPITX_DSI_PHY_SEL, MIPI_TX_LPRX_SEL,dsi_params->lane_swap[i][MIPITX_PHY_LANE_RX]);
		}
		...
		{
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV0,dsi_params->pll_div1);
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON0,RG_DSI0_MPPLL_TXDIV1,dsi_params->pll_div2);
		
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_H,((dsi_params->fbk_div)<< 2));
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_16_23,0);
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_8_15,0);
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON2_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2,RG_DSI0_MPPLL_SDM_PCW_0_7,0);
		...
		if(dsi_params->lane_swap_en)
		{
			...
			{
				MIPITX_OUTREGBIT(MIPITX_DSI_CLOCK_LANE_REG,DSI_PHY_REG[i]->MIPITX_DSI_CLOCK_LANE,RG_DSI_LNTC_CKLANE_EN,1);	
			}
			else
			{
				MIPITX_OUTREGBIT(MIPITX_DSI_CLOCK_LANE_REG,DSI_PHY_REG[i]->MIPITX_DSI_CLOCK_LANE,RG_DSI_LNTC_CKLANE_EN,0);	
			}
		}
		...
		if (dsi_params->LANE_NUM > 0) 
		{
			...
			if(dsi_params->lane_swap_en)
			{
				...
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE0_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE0,RG_DSI_LNT0_CKLANE_EN,1);	
				}
				else
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE0_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE0,RG_DSI_LNT0_CKLANE_EN,0);	
				}
			}
		}
		/* step 13 */
		if (dsi_params->LANE_NUM > 1) 
		{
			...
			if(dsi_params->lane_swap_en)
			{
				...
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE1_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE1,RG_DSI_LNT1_CKLANE_EN,1);	
				}
				else
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE1_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE1,RG_DSI_LNT1_CKLANE_EN,0);	
				}
			}
		}
		/* step 14 */
		if (dsi_params->LANE_NUM > 2) 
		{
			...
			if(dsi_params->lane_swap_en)
			{
				...
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE2_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE2,RG_DSI_LNT2_CKLANE_EN,1);	
				}
				else
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE2_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE2,RG_DSI_LNT2_CKLANE_EN,0);	
				}
			}		
		}
		/* step 15 */
		if (dsi_params->LANE_NUM > 3) 
		{
			...
			if(dsi_params->lane_swap_en)
			{
				...
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE3_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE3, RG_DSI_LNT3_CKLANE_EN,1); 
				}
				else
				{
					MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE3_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE3, RG_DSI_LNT3_CKLANE_EN,0); 
				}
			}
		}
		/* step 16 */
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	hs_trail_m=1;
	hs_trail_n= (dsi_params->HS_TRAIL == 0) ? NS_TO_CYCLE(((hs_trail_m * 0x4) + 60), cycle_time) : dsi_params->HS_TRAIL;
	...
	// --------------------------------------------------------------
	timcon1.DA_HS_EXIT  = (dsi_params->DA_HS_EXIT == 0) ? NS_TO_CYCLE((0x3c + 0x80 * ui), cycle_time) : dsi_params->DA_HS_EXIT;

	timcon2.CLK_TRAIL	= ((dsi_params->CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64, cycle_time) : dsi_params->CLK_TRAIL) + 0x0a;
	...
	timcon3.CLK_HS_POST= (dsi_params->CLK_HS_POST == 0) ? NS_TO_CYCLE((0x3c + 0x80 * ui), cycle_time) : dsi_params->CLK_HS_POST;
...
UINT32 DSI_dcs_read_lcm_reg_v2(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
	...
	for(d = DSI_MODULE_BEGIN(module);d <= DSI_MODULE_END(module);d++)
	{
		...
		do
		{
			...
			/// 4: read data
			ret = wait_event_interruptible_timeout(_dsi_dcs_read_wait_queue[d], !(DSI_REG[d]->DSI_INTSTA.BUSY), WAIT_TIMEOUT);
...
void DSI_set_cmdq_V22_Wrapper_DSI0(void* cmdq,unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	DSI_set_cmdq_V2(DISP_MODULE_DSI0, cmdq, cmd, count, para_list, force_update);
}

void DSI_set_cmdq_V22_Wrapper_DSI1(void* cmdq,unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	DSI_set_cmdq_V2(DISP_MODULE_DSI1, cmdq, cmd, count, para_list, force_update);
}

void DSI_set_cmdq_V22_Wrapper_DSIDual(void* cmdq,unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
...
void DSI_set_cmdq_V2_DSIx(void* cmdq, unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
...
int ddp_dsi_start(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	DISPMSG("ddp_dsi_start, x=%d, y=%d\n", g_lcm_x, g_lcm_y);
	int i = 0;
	DISPFUNC();
	if(dprec_option_enabled())
	{
		DISPMSG("dump before ddp_dsi_start\n");
		DSI_DumpRegisters(module,2);
	}
	...
	if(module == DISP_MODULE_DSIDUAL)
	{
	    ...
	    {	       
	       DSI_Send_ROI(DISP_MODULE_DSI0, cmdq, g_lcm_x, g_lcm_y, _dsi_context[i].lcm_width, _dsi_context[i].lcm_height);
	    }else{
           DSI_Send_ROI(DISP_MODULE_DSI1, cmdq, g_lcm_x, g_lcm_y, _dsi_context[i].lcm_width, _dsi_context[i].lcm_height);
	    }
	...
	if(dprec_option_enabled())
	{
		DISPMSG("dump after ddp_dsi_start\n");	
		DSI_DumpRegisters(module,2);
	}
...
int ddp_dsi_switch_lcm_mode(DISP_MODULE_ENUM module, void* params)
{
	...
	{
		DISPMSG("[ddp_dsi_switch_mode] not need switch mode, current mode = %d, switch to %d\n", dsi_currect_mode, mode);
		return 0;
	}
...
int ddp_dsi_switch_mode(DISP_MODULE_ENUM module, void *cmdq_handle, void* params)
{
	...
	if(mode == 0)
	{
		DISPMSG("v2c switch begin\n");
	#if 1
		DSI_SetBypassRack(module, cmdq_handle, 1);

		DSI_OUTREG32(cmdq_handle, (unsigned long)(DSI_REG[i])+0x130, 0x00001521|(lcm_cmd.addr << 16)|(lcm_cmd.val[0]<< 24)); // RM = 1
...
//		DSI_SetBypassRack(module, NULL, 0);
	...
	#endif
		DISPMSG("v2c switch finished\n");
	}
	else
	{
		DISPMSG("c2v switch begin\n");
	#if 0
		cmdqRecWaitNoClear(cmdq_handle, CMDQ_SYNC_TOKEN_STREAM_EOF);
		...
		DSI_OUTREG32(cmdq_handle,  (unsigned int)(DSI_REG[i])+0x200+0, 0x00001500 |(lcm_cmd.addr << 16)|(lcm_cmd.val[0]<< 24));
		...
		//wait_cmd_done = false;
		cmdqRecFlush(cmdq_handle);
		DISPMSG("after c2v switch, cmdq flushed\n");
		//msleep(16);
		#if 1
		...
		while(wait_count)
		{
			...
			{
				DISPMSG("after c2v switch, dsi fsm is idle\n");
				break;
			}
			msleep(1);
			wait_count--;
...
int ddp_dsi_ioctl(DISP_MODULE_ENUM module, void *cmdq_handle, unsigned int ioctl_cmd, unsigned long *params)
{
	...
	switch(ioctl)
	{
		...
		case DDP_BACK_LIGHT:
		{
			...
			printk("[ddp_dsi_ioctl] level = %d module %d\n", level,module);
			if(module == DISP_MODULE_DSIDUAL)
			{
#if 1			
				if(dsi_currect_mode==CMD_MODE)
				{
					...
					DSI_OUTREGBIT(cmdq_handle, DSI_START_REG,DSI_REG[0]->DSI_START,DSI_START,0);
					DSI_OUTREGBIT(cmdq_handle, DSI_START_REG,DSI_REG[1]->DSI_START,DSI_START,0);
					DSI_OUTREGBIT(cmdq_handle, DSI_COM_CTRL_REG,DSI_REG[0]->DSI_COM_CTRL,DSI_DUAL_EN,1);
					DSI_OUTREGBIT(cmdq_handle, DSI_COM_CTRL_REG,DSI_REG[1]->DSI_COM_CTRL,DSI_DUAL_EN,1);
				}
#else			
				printk("setbacklight module=%d\n",module);
				DSI_OUTREGBIT(cmdq_handle, DSI_START_REG,DSI_REG[0]->DSI_START,DSI_START,0);
				DSI_OUTREGBIT(cmdq_handle, DSI_START_REG,DSI_REG[1]->DSI_START,DSI_START,0);
				if(_dsi_context[0].swap_port || _dsi_context[1].swap_port)
				{
...
void dsi_analysis(DISP_MODULE_ENUM module)
{
	...
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
		...
		DDPDUMP("DSI%d IRQ,RD_RDY:%d, CMD_DONE:%d, EXT_TE:%d, SLEEPOUT_DONE:%d, TE_RDY:%d, VM_CMD_DONE:%d, VM_DONE:%d\n", i,
...
//			ret = cmdqRecPoll(cmdq_trigger_handle, 0x1401c00c, 0, 0x80000000);
	...
	{	
		...
		for(i = 0; i < 3; i++)
		{
	        ...
	        if(dsi_i==0) // DSI0
	        {
		        ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[0])+0xc), 0x1, 0x00000001);
		        ret = cmdqRecWrite(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[0])+0xc), 0x0, 0x00000001);
    		}
			else // DSI1
			{			
		        ret = cmdqRecPoll(cmdq_trigger_handle,  disp_addr_convert((unsigned long)(DSI_REG[1])+0xc), 0x1, 0x00000001);
		        ret = cmdqRecWrite(cmdq_trigger_handle,  disp_addr_convert((unsigned long)(DSI_REG[1])+0xc), 0x0, 0x00000001);
			}

	        // 2. save RX data 
	        if(hSlot)
	        {
                cmdqRecBackupRegisterToSlot(cmdq_trigger_handle, hSlot, i, dsi_i ?  disp_addr_convert((unsigned long)(DSI_REG[1])+0x74) : disp_addr_convert((unsigned long)(DSI_REG[0])+0x74));
	        }
			...
			if(dsi_i==0) // DSI0
			{
				ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[0])+0x0c), 0, 0x80000000);
			}
			else // DSI1
			{
				ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[1])+0x0c), 0, 0x80000000);
			}
    		// loop: 0~4
		}
		if(module == DISP_MODULE_DSIDUAL)
