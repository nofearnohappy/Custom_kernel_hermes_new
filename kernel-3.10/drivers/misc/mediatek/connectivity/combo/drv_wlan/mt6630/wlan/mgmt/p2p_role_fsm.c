...
VOID p2pRoleFsmRunEventTimeout(IN P_ADAPTER_T prAdapter, IN ULONG ulParamPtr)
{
	...
	do {
		...
		case P2P_ROLE_STATE_IDLE:
			...
			if (prP2pChnlReqInfo->fgIsChannelRequested) {
				p2pFuncReleaseCh(prAdapter,
						 prP2pRoleFsmInfo->ucBssIndex, prP2pChnlReqInfo);
		...
		case P2P_ROLE_STATE_GC_JOIN:
			DBGLOG(P2P, ERROR,
			       ("Current P2P Role State P2P_ROLE_STATE_GC_JOIN is unexpected for FSM timeout event(wait SAA msg...).\n"));
			break;
		default:
			DBGLOG(P2P, ERROR,
...
VOID
p2pRoleFsmRunEventRxDeauthentication(IN P_ADAPTER_T prAdapter,
...
VOID
p2pRoleFsmRunEventRxDisassociation(IN P_ADAPTER_T prAdapter,
				   IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prSwRfb)
{
	P_BSS_INFO_T prP2pBssInfo = (P_BSS_INFO_T) NULL;
	UINT_16 u2ReasonCode = 0;
	...
	do {
		...
		if (prP2pBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
			...
			kalP2PGCIndicateConnectionStatus(prAdapter->prGlueInfo,
							 prP2pRoleFsmInfo->ucRoleIndex,
							 NULL,
							 NULL, 0, REASON_CODE_DISASSOC_INACTIVITY);
		...
		case OP_MODE_ACCESS_POINT:
			{
                ...
                prCurrStaRec = bssRemoveClientByMac(prAdapter,
                    prP2pBssInfo, prDisconnMsg->aucTargetID);
			...
			if (prJoinCompMsg->ucSeqNum == prJoinInfo->ucSeqNumOfReqMsg) {
				...
				if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
					...
					if (p2pFuncRetryJOIN(prAdapter, prStaRec, prJoinInfo) ==
						...
						kalP2PGCIndicateConnectionStatus(prAdapter->
										 prGlueInfo,
										 prP2pRoleFsmInfo->
										 ucRoleIndex,
										 &prP2pRoleFsmInfo->
...
VOID
p2pRoleFsmRunEventScanDone(IN P_ADAPTER_T prAdapter,
			   IN P_MSG_HDR_T prMsgHdr, IN P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo)
{
	...
	do {
		ASSERT_BREAK((prAdapter != NULL) &&
			     (prMsgHdr != NULL) && (prP2pRoleFsmInfo != NULL));
...
p2pRoleFsmRunEventAAAComplete(IN P_ADAPTER_T prAdapter,
			      IN P_STA_RECORD_T prStaRec, IN P_BSS_INFO_T prP2pBssInfo)
{
...
/*----------------------------------------------------------------------------*/
VOID
p2pRoleFsmRunEventAAATxFail(IN P_ADAPTER_T prAdapter,
			    IN P_STA_RECORD_T prStaRec, IN P_BSS_INFO_T prP2pBssInfo)
{
	ASSERT(prAdapter);
	ASSERT(prStaRec);
