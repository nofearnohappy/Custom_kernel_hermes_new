...
static const UINT_16     g_u2CountryGroup7[] = {
    ...
    COUNTRY_CODE_NE,    COUNTRY_CODE_TN,    COUNTRY_CODE_EH
};
...
{
    UINT_8   								i, j;
	UINT_16  								u2CountryCodeTable;
	BOOLEAN                                 fgChannelValid = FALSE;
	BOOLEAN                                 fgPowerLimitValid = FALSE;
	BOOLEAN                                 fgTableValid = TRUE;
    P_CHANNEL_POWER_LIMIT                     prChPwrLimit;
	for (i = 0; i < MAX_COUNTRY_NUM; i++) {
            /*Table Country Code*/
			u2CountryCodeTable = (((UINT_16) g_rRlmCountryPowerLimitTable[i].aucCountryCode[0]) << 8) |	 \
							(((UINT_16) g_rRlmCountryPowerLimitTable[i].aucCountryCode[1]) & BITS(0,7));
			
            for (j = 0; j < MAX_CHANNEL_NUM ; j++) {
				prChPwrLimit = &g_rRlmCountryPowerLimitTable[i].rChannelPowerLimit[j];
				/*Channel Number Check*/
				fgChannelValid = rlmDomainCheckChannelEntryValid(prAdapter, prChPwrLimit->ucCentralCh);

				/*Power Limit Check*/
				fgPowerLimitValid = rlmDomainCheckPowerLimitValid(prAdapter, &prChPwrLimit->cPwrLimitCCK, POWER_LIMIT_NUM);

				if (fgChannelValid == FALSE || fgPowerLimitValid == FALSE) {
					fgTableValid = FALSE;
					DBGLOG(RLM, INFO, ("Domain: CC:0x%04x,Ch:%d, Limit: %d,%d,%d,%d,%d\n",
					u2CountryCodeTable,prChPwrLimit->ucCentralCh,prChPwrLimit->cPwrLimitCCK, 
					prChPwrLimit->cPwrLimit20, prChPwrLimit->cPwrLimit40, prChPwrLimit->cPwrLimit80,
					prChPwrLimit->cPwrLimit160));
				}
			}
...
/*----------------------------------------------------------------------------*/
UINT_16
rlmDomainPwrLimitTableDecision(
    P_ADAPTER_T     prAdapter,
    ...
    )
{
	...
	for (i = 0; i < MAX_COUNTRY_NUM; i++) {
			...
			if (u2CountryCodeTable == u2CountryCode) {
                u2TableIndex = i;
				break; /*Match the country table entry*/
			}
			else if(u2CountryCodeTable == COUNTRY_CODE_NULL) {
				DBGLOG(RLM, INFO, ("Domain: no match country\n"));
				break; /*End of country table entry*/
			}
	}
    ...
    )
{
    ...
    u2TableIndex = rlmDomainPwrLimitTableDecision(prAdapter, prAdapter->rWifiVar.rConnSettings.u2CountryCode);

	u4SetCmdTableMaxSize = sizeof(CMD_SET_COUNTRY_CHANNEL_POWER_LIMIT_T) + MAX_CHANNEL_NUM * sizeof(CMD_CHANNEL_POWER_LIMIT);
	...
	if (u2TableIndex != POWER_LIMIT_TABLE_NULL && u2TableIndex < MAX_COUNTRY_NUM) {
		...
		for (i = 0; i < MAX_CHANNEL_NUM; i++) {
			...
			if(prChPwrLimit->ucCentralCh != ENDCH){
			    ...
			    if (fgChannelValid) {
						...
						DBGLOG(RLM, INFO, ("Domain: Non-Ch=%d,Limit=%d,%d,%d,%d,%d,Fg=%d\n",prChPwrLimit->ucCentralCh,
				    	prChPwrLimit->cPwrLimitCCK,prChPwrLimit->cPwrLimit20,prChPwrLimit->cPwrLimit40,
						prChPwrLimit->cPwrLimit80,prChPwrLimit->cPwrLimit160,prChPwrLimit->ucFlag));
				}
				prChPwrLimit++;
			}
			else {
	...
	u4SetQueryInfoLen = (sizeof(CMD_SET_COUNTRY_CHANNEL_POWER_LIMIT_T)+ (prCmd->ucNum)*sizeof(CMD_CHANNEL_POWER_LIMIT)); 

	DBGLOG(RLM, INFO, ("Domain: ValidCC= 0x%04x, ChNum=%d\n",prCmd->u2CountryCode,prCmd->ucNum));
	
    /* Update domain info to chip */
	if (prCmd->ucNum < MAX_CHANNEL_NUM) {
