...
WLAN_STATUS nicAllocateAdapterMemory(IN P_ADAPTER_T prAdapter)
{
    ...
    do {
        ...
        LOCAL_NIC_ALLOCATE_MEMORY(prAdapter->pucMgtBufCached,
					  ...
					  PHY_MEM_TYPE, "COMMON MGMT MEMORY POOL");
        ...
        LOCAL_NIC_ALLOCATE_MEMORY(prRxCtrl->pucRxCached,
					  prRxCtrl->u4RxCachedSize, VIR_MEM_TYPE, "SW_RFB_T");
        ...
        LOCAL_NIC_ALLOCATE_MEMORY(prTxCtrl->pucTxCached,
					  prTxCtrl->u4TxCachedSize, VIR_MEM_TYPE, "MSDU_INFO_T");
		...
		prAdapter->u4CoalescingBufCachedSize =
		    ...
		    CFG_RX_COALESCING_BUFFER_SIZE ? CFG_COALESCING_BUFFER_SIZE :
...
VOID nicReleaseAdapterMemory(IN P_ADAPTER_T prAdapter)
{
    ...
    if (prAdapter->prSDIOCtrl) {
		kalReleaseIOBuffer((PVOID) prAdapter->prSDIOCtrl,
				   sizeof(ENHANCE_MODE_DATA_STRUCT_T));
		prAdapter->prSDIOCtrl = (P_SDIO_CTRL_T) NULL;
    }
	/* 4 <4> Memory for Common Coalescing Buffer */
    ...
    if (prAdapter->pucCoalescingBufCached) {
		kalReleaseIOBuffer((PVOID) prAdapter->pucCoalescingBufCached,
				   prAdapter->u4CoalescingBufCachedSize);
		prAdapter->pucCoalescingBufCached = (PUINT_8) NULL;
    }
#endif /* CFG_COALESCING_BUFFER_SIZE */
...
/*----------------------------------------------------------------------------*/
P_MSDU_INFO_T
nicGetPendingTxMsduInfo(IN P_ADAPTER_T prAdapter, IN UINT_8 ucWlanIndex, IN UINT_8 ucPID)
{
    P_QUE_T prTxingQue;
    QUE_T rTempQue;
...
nicMediaStateChange(IN P_ADAPTER_T prAdapter,
		    IN UINT_8 ucBssIndex, IN P_EVENT_CONNECTION_STATUS prConnectionStatus)
{
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
nicMediaJoinFailure(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, IN WLAN_STATUS rStatus)
{
    P_GLUE_INFO_T prGlueInfo;
...
WLAN_STATUS nicActivateNetwork(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	if (EQUAL_MAC_ADDR(aucZeroMacAddr, prBssInfo->aucBSSID)) {
		prBssInfo->ucBMCWlanIndex =
		    secPrivacySeekForBcEntry(prAdapter, prBssInfo->ucBssIndex,
					     prBssInfo->aucOwnMacAddr, 0xff, CIPHER_SUITE_NONE,
					     0xff, 0x0, BIT(0));
	} else {
		prBssInfo->ucBMCWlanIndex =
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            sizeof(CMD_BSS_ACTIVATE_CTRL),
				   (PUINT_8) & rCmdActivateCtrl, NULL, 0);
}
...
WLAN_STATUS nicPmIndicateBssCreated(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            sizeof(CMD_INDICATE_PM_BSS_CREATED),
				   (PUINT_8) & rCmdIndicatePmBssCreated, NULL, 0);
}
...
WLAN_STATUS nicPmIndicateBssAbort(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            sizeof(CMD_INDICATE_PM_BSS_ABORT),
				   (PUINT_8) & rCmdIndicatePmBssAbort, NULL, 0);
}

WLAN_STATUS
nicConfigPowerSaveProfile(IN P_ADAPTER_T prAdapter,
    IN UINT_8 ucBssIndex,
			  IN PARAM_POWER_MODE ePwrMode, IN BOOLEAN fgEnCmdEvent)
{
...
WLAN_STATUS nicQmUpdateWmmParms(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            sizeof(CMD_UPDATE_WMM_PARMS_T),
				   (PUINT_8) & rCmdUpdateWmmParms, NULL, 0);
}
...
WLAN_STATUS nicUpdateTxPower(IN P_ADAPTER_T prAdapter, IN P_CMD_TX_PWR_T prTxPwrParam)
{
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            FALSE,
            NULL,
				   NULL, sizeof(CMD_TX_PWR_T), (PUINT_8) prTxPwrParam, NULL, 0);
}
...
WLAN_STATUS nicUpdate5GOffset(IN P_ADAPTER_T prAdapter, IN P_CMD_5G_PWR_OFFSET_T pr5GPwrOffset)
{
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
				   sizeof(CMD_5G_PWR_OFFSET_T), (PUINT_8) pr5GPwrOffset, NULL, 0);
#else
    return 0;
#endif
}
...
WLAN_STATUS nicUpdateDPD(IN P_ADAPTER_T prAdapter, IN P_CMD_PWR_PARAM_T prDpdCalResult)
{
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
				   sizeof(CMD_PWR_PARAM_T), (PUINT_8) prDpdCalResult, NULL, 0);
}
		 ...
		 IN PARAM_RATES_EX rSupportedRates, IN UINT_16 u2IELength, IN PUINT_8 pucIEBuf)
{
	...
	u4BufferSize =
	    sizeof(prAdapter->rWlanInfo.aucScanIEBuf) /
	    sizeof(prAdapter->rWlanInfo.aucScanIEBuf[0]);
		...
		if (prAdapter->rWlanInfo.arScanResult[i].eOpMode == eOpMode &&
		    ...
		    (EQUAL_SSID
		     (prAdapter->rWlanInfo.arScanResult[i].rSsid.aucSsid,
		      prAdapter->rWlanInfo.arScanResult[i].rSsid.u4SsidLen, prSsid->aucSsid,
                            prSsid->u4SsidLen)
			...
			if (u2IELength > 0) {
				if (ALIGN_4(u2IELength) +
				    prAdapter->rWlanInfo.u4ScanIEBufferUsage <= u4BufferSize) {
					kalMemCopy(&
						   (prAdapter->rWlanInfo.
						    aucScanIEBuf[prAdapter->rWlanInfo.
								 u4ScanIEBufferUsage]), pucIEBuf,
                            u2IELength);
...
VOID nicFreeScanResultIE(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Idx)
{
	...
	pucMovePivot =
	    (PUINT_8) ((ULONG) (prAdapter->rWlanInfo.apucScanResultIEs[u4Idx]) + u4FreeSize);
...
WLAN_STATUS nicWriteMcr(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Address, IN UINT_32 u4Value)
{
    ...
    return wlanSendSetQueryCmd(prAdapter,
                ...
                NULL,
                NULL,
				   sizeof(CMD_ACCESS_REG), (PUINT_8) &rCmdAccessReg, NULL, 0);
...
nicUpdateLinkQuality(IN P_ADAPTER_T prAdapter,
		     IN UINT_8 ucBssIndex, IN P_EVENT_LINK_QUALITY_V2 prEventLinkQuality)
{
    INT_8    cRssi;
	...
	switch (GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eNetworkType) {
    case NETWORK_TYPE_AIS:
		if (GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eConnectionState ==
            ...
            if(prAdapter->fgIsLinkQualityValid == FALSE
                ...
                cRssi = prEventLinkQuality->rLq[ucBssIndex].cRssi; // ranged from (-128 ~ 30) in unit of dBm
                cRssi = (INT_8)( ( (INT_16)(cRssi) * u2AdjustRssi) / 10 ); 
                DBGLOG(RLM,INFO,("Rssi=%d, NewRssi=%d\n",prEventLinkQuality->rLq[ucBssIndex].cRssi,cRssi));
                nicUpdateRSSI(prAdapter, ucBssIndex, cRssi, prEventLinkQuality->rLq[ucBssIndex].cLinkQuality);
            }
    ...
    case NETWORK_TYPE_P2P:
        if (prAdapter->fgIsP2pLinkQualityValid == FALSE
		    ...
		    CFG_LINK_QUALITY_VALID_PERIOD) {
			P_EVENT_LINK_QUALITY_EX prEventLQEx =
			    (P_EVENT_LINK_QUALITY_EX) prEventLinkQuality;

			nicUpdateRSSI(prAdapter, ucBssIndex, prEventLQEx->cRssiP2P,
...
/*----------------------------------------------------------------------------*/
VOID
nicUpdateRSSI(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, IN INT_8 cRssi, IN INT_8 cLinkQuality)
{
    ASSERT(prAdapter);
    ASSERT(ucBssIndex <= MAX_BSS_INDEX);
...
/* aisFsmScanRequest(prAdapter, NULL); */
    ...
    return wlanSendSetQueryCmd(prAdapter,
            ...
            FALSE,
            NULL,
				   NULL, sizeof(CMD_RDD_CH_T), (PUINT_8) prRddChParam, NULL, 0);
}
#endif
