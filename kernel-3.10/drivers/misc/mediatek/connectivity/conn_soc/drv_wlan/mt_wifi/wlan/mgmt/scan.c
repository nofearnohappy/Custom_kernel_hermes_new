	...
	else
	{
    	...
    	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
						   ...
						   //4 Auto Channel Selection:Record the AP Number
							   ...
							   if ((prBssDesc->ucChannelNum>0) && (prBssDesc->ucChannelNum<=48) || prBssDesc->ucChannelNum>= 147/*non-DFS Channel*/)
							   {
								   if( prBssDesc->ucChannelNum<=HW_CHNL_NUM_MAX_2G4) {
									   ...
									   ucIdx=(UINT_8)(HW_CHNL_NUM_MAX_2G4+(prBssDesc->ucChannelNum-34)/4);
								   	   ...
								   	   ucIdx=(UINT_8)(HW_CHNL_NUM_MAX_2G4+4+(prBssDesc->ucChannelNum-149)/4);
								   }
        ...
        if (prStaRec) {
            ...
            if (prStaRec->u2StatusCode != STATUS_CODE_SUCCESSFUL) {
                ...
                if ((prStaRec->ucJoinFailureCount < JOIN_MAX_RETRY_FAILURE_COUNT) ||
                    ...
                    DBGLOG(SCN, INFO, ("SEARCH: Try to join BSS again which has Status Code = %d (Curr = %u/Last Join = %u)\n",
