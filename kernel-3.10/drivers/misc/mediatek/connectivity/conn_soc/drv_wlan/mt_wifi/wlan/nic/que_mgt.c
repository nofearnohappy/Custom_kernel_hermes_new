...
if (prDequeuedPkt != NULL)
{
   ...
   if (*pu4HeadStaRecForwardCount){
        DBGLOG(QM, LOUD, ("TC = %u Round Head STA = %u, u4HeadStaRecForwardCount = %u\n", ucTC, *pu4HeadStaRecIndex, (*pu4HeadStaRecForwardCount)));
   }
    ...
    )
{
    ...
    */
	/* tx done -> nicProcessTxInterrupt() -> nicTxAdjustTcq() -> qmAdjustTcQuotas() -> check fgTcResourcePostAnnealing */
    ...
    )
{
    ...
    if  /* 0 - start - sn - end - 4095 */
        ...
        if (qmPopOutDueToFallWithin(prReorderQueParm, prReturnedQue) == FALSE)
